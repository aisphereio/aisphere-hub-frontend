...
// ─── Skill File CRUD (draft version editing) ────────────────────────
...
export function useSaveSkillFile() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      content: string;
      commitMsg?: string;
    }) => skillApi.saveFile(skillName, version, path, content, commitMsg),
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({
        queryKey: ["skills", "files", vars.skillName, vars.version],
      });
      queryClient.invalidateQueries({
        queryKey: ["skills", "file", vars.skillName, vars.version, vars.path],
      });
    },
  });
}
...
export function useCreateSkillFile() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      type?: "file" | "dir";
      content?: string;
    }) => skillApi.createFile(skillName, version, path, type ?? "file", content ?? ""),
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({
        queryKey: ["skills", "files", vars.skillName, vars.version],
      });
...
export function useDeleteSkillFile() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      version: string;
      path: string;
    }) => skillApi.deleteFile(skillName, version, path),
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({
        queryKey: ["skills", "files", vars.skillName, vars.version],
      });
...
export function useRenameSkillFile() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      oldPath: string;
      newPath: string;
    }) => skillApi.renameFile(skillName, version, oldPath, newPath),
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({
        queryKey: ["skills", "files", vars.skillName, vars.version],
      });
...
export function useEnsureDraftVersion() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ skillName, baseVersion }: { skillName: string; baseVersion?: string }) =>
      skillApi.ensureDraftVersion(skillName, baseVersion ?? ""),
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({
